...
/**
 * @brief System Control Block (SCB) register structure
 * @note ARM Cortex-M4 core registers per ARM documentation
 */
typedef struct {
    ...
    uint32_t RESERVED0[5U];            /**< Reserved */
...
/*==============================================================================
 * HARDWARE BASE ADDRESSES (STM32F407VG MEMORY MAP)
 *============================================================================*/
#define SCB_BASE            (0xE000ED00UL)    /**< System Control Block base */
#define RCC_BASE            (0x40023800UL)    /**< Reset Clock Control base */
#define FLASH_R_BASE        (0x40023C00UL)    /**< Flash interface base */
...
/**
 * @brief High Speed External (HSE) oscillator frequency
 * @note STM32F4 Discovery board uses 8MHz crystal
 */
#define HSE_VALUE           (8000000UL)

/**
 * @brief High Speed Internal (HSI) oscillator frequency  
 * @note Internal RC oscillator, less accurate than HSE
 */
...
#define RCC_CR_HSEON        (0x00010000UL)    /**< HSE oscillator enable */
#define RCC_CR_HSERDY       (0x00020000UL)    /**< HSE oscillator ready */
#define RCC_CR_PLLON        (0x01000000UL)    /**< PLL enable */
#define RCC_CR_PLLRDY       (0x02000000UL)    /**< PLL ready */

/* RCC Clock Configuration Register bits */
...
#define FLASH_ACR_LATENCY_5WS   (0x00000005UL)    /**< 5 wait states for 168MHz */
...
/**
 * @brief Perform system clock monitoring and fault detection
 * @return DSRTOS_SUCCESS if healthy, error code if fault detected
 * @note Continuous monitoring for clock failures
 */
static dsrtos_error_t monitor_clock_health(void);
...
static dsrtos_error_t monitor_clock_health(void)
{
    ...
    current_source = RCC->CFGR & RCC_CFGR_SWS_MASK;
    
    /* Verify clock source hasn't changed unexpectedly */
    if (current_source != active_clock_source) {
        result = DSRTOS_ERR_CLOCK_FAULT;
    }
    
    /* Check HSE oscillator if PLL is active */
    if ((active_clock_source == RCC_CFGR_SWS_PLL) && 
        ((RCC->CR & RCC_CR_HSERDY) == 0UL)) {
        result = DSRTOS_ERR_CLOCK_FAULT;
    }
    
    /* Check PLL lock if PLL is active */
    if ((active_clock_source == RCC_CFGR_SWS_PLL) && 
        ((RCC->CR & RCC_CR_PLLRDY) == 0UL)) {
        result = DSRTOS_ERR_CLOCK_FAULT;
    }
