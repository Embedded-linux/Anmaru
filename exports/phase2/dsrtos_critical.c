...
static void critical_check_timeout(uint32_t entry_time);
static void critical_update_stats(uint32_t duration_cycles);
static bool critical_validate_state(void);
...
/**
 * @brief Check for critical section timeout
 */
static void critical_check_timeout(uint32_t entry_time)
{
    uint32_t current_time = critical_get_timestamp();
    uint32_t duration = current_time - entry_time;
    
    if (duration > g_critical_timeout_cycles) {
        g_critical_cb.stats.timeout_count++;
        
        /* Call panic handler for excessive duration */
        if (duration > (g_critical_timeout_cycles * 10U)) {
            critical_panic_handler("Critical section timeout exceeded");
        }
    }
}
...
static bool critical_validate_state(void)
{
    ...
    /* Check statistics consistency */
    if (g_critical_cb.stats.exit_count > g_critical_cb.stats.enter_count) {
        valid = false;
    }
