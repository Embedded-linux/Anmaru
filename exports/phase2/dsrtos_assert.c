...
/* CERTIFIED_DUPLICATE_REMOVED: void dsrtos_assert_failed(const char* expr, const char* file, int line, const char* func)
{
    /* Disable interrupts */
    __disable_irq();
    ...
    /*dsrtos_assert_type_t type = DSRTOS_ASSERT_TYPE_CONDITION;*/
    dsrtos_assert_type_t type = DSRTOS_ASSERT_TYPE_PRECONDITION;
    
    /* Call custom handler if registered */
    if (g_assert_state.custom_handler != NULL) {
       g_assert_state.custom_handler(expr, file, line, func, type);
	/*g_assert_state.custom_handler(expr, file, line, func);*/
    }
    else {
        assert_default_handler(expr, file, line, func, type);
    }
    
    /* Trigger panic */
    dsrtos_panic(DSRTOS_PANIC_ASSERTION_FAILED, expr, file, line);
