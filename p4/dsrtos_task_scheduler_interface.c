...
static void update_queue_checksum(dsrtos_ready_queue_t* queue);
static bool detect_priority_inversion(const dsrtos_tcb_t* task);
    ...
    const dsrtos_scheduler_ops_t* ops)
{
    ...
    if ((interface == NULL) || (ops == NULL)) {
        DSRTOS_ASSERT(false, "Null pointer in scheduler init");
    ...
    dsrtos_tcb_t* task)
{
    ...
    if (!dsrtos_ready_queue_validate(queue)) {
        ...
        if (result != DSRTOS_SUCCESS) {
            DSRTOS_ASSERT(false, "Queue corrupted and repair failed");
            return DSRTOS_ERROR_CORRUPTED;
        }
    }
    ...
    if (priority_list->count >= priority_list->max_count) {
        ...
        dsrtos_ready_queue_unlock(queue);
        return DSRTOS_ERROR_QUEUE_FULL;
    }
...
void dsrtos_priority_bitmap_set(volatile uint32_t* bitmap, uint8_t priority)
{
    ...
    if ((bitmap == NULL) || (priority >= DSRTOS_PRIORITY_LEVELS)) {
        DSRTOS_ASSERT(false, "Invalid bitmap operation");
        return;
    }
